...
class GuidedPolicy:
    ...
    def __call__(self, conditions, debug=False, batch_size=1):
        ...
        sample, value= self.diffusion_model(conditions, guide = self.guide, **self.sample_kwargs)
        ...
        trajectories = GuidedTrajectories(actions, observations, value)
        return action, trajectories

class GuidedPolicy_diffusion(GuidedPolicy):
    ...
    def __call__(self, conditions, debug=False, batch_size=1):
        ...
        ## run reverse diffusion process
        #sample, value= self.diffusion_model(conditions, guide = self.guide, **self.sample_kwargs)
        samples= self.diffusion_model(conditions, guide = self.guide, **self.sample_kwargs)
        sample = samples.trajectories
        value = samples.values
        chains = samples.chains
        normed_obs_chains = samples.chains[:,:,:,self.action_dim:]
        normed_obs_chains = utils.to_np(normed_obs_chains)
        #print(f'obs_chains{normed_obs_chains.device}')
        obs_chains = self.normalizer.unnormalize(normed_obs_chains, 'observations')
        #trajectories values chains
        sample = utils.to_np(sample)
        ...
        # if debug:
        normed_observations = sample[:, :, self.action_dim:]
        #print(f'obs{normed_observations.device}')
        observations = self.normalizer.unnormalize(normed_observations, 'observations')
        ...
        #trajectories = GuidedTrajectories(actions, observations, value)
        #print('haha')
        trajectories = GuidedTrajectories_diffusion(actions, observations, value, obs_chains)
