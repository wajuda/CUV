...
#以上一段轨迹终点为起点， 固定终点，看是否能够绕过block达到终点
...
class Parser(utils.Parser):
    ...
    config: str = 'config.maze2d_block'
    exp: str = 'blocked' #如果到了一个点被block住了，重新规划能到达吗？
...
while valid_episode < total_episode:
    if valid_episode == 0:
        rollout, sequence, terminal= loop(valid_episode)
    else:
    ...
    valid_episode += 1
    if terminal:
        print(f'Episode {valid_episode} reached the target.')
        break
