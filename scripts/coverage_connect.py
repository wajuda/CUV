...
class Parser(utils.Parser):
    ...
    exp: str = 'short' #如果到了一个点被block住了，重新规划能到达吗？
...
def loop(episode=0, plot = False, point = np.array([1.0, 1.0])):
    #---------------------------------- one loop ----------------------------------#
    actions_rollout = np.empty((0, 2), dtype=np.float32)

    #observation = env.reset()
    if point:
        observation = env.reset_to_location(point)  # [x, y, vx, vy]
    else:
        observation = env.reset()
    ...
    target = observation[:2] + np.random.uniform(-0.1, 0.1, size=2) + np.array([0,1])  # [x, y] for maze2d
    env._target = target  # [x, y] for maze2d
    ...
    return rollout, sequence, terminal
#loop()
rollouts = []
sequences = []
total_num = 0
...
while valid_episode < total_episode:
    if valid_episode == 0:
        rollout, sequence, terminal= loop(valid_episode, point = None)
    else:
        #rollout, sequence = loop(valid_episode, plot=True)
        rollout, sequence, terminal = loop(valid_episode, plot=False, point=np.array(rollouts[-1][-1][:2]))
...
renderer.composite(join(args.savepath, f'rollouts_cover_{valid_episode}.png'), rollouts, ncol=1)
renderer.composite(join(args.savepath, f'sequences_cover_{valid_episode}.png'), sequences, ncol=1)
